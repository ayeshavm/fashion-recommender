...
# Future? # add swim categories later "skort":}
# ---- Helper functions -----
...
def get_similar_items(uploaded_image):
    # 🧠 Placeholder for your FAISS/CLIP similarity search
    # Return a list of image paths for similar items
    return []  # You can

# detected_type = detect_item_type(uploaded_file.name)
...
st.title("Rosée AI 🌸 - Your Style Ally")
    ...
    if st.session_state.selected_image is None:
        ...
        for idx, image_name in enumerate(gallery_images):
            ...
            with cols[idx % 3]:
                ...
                if st.button(f"Select {image_name}", key=f"btn_{image_name}"):
...
if display_img is not None:
    ...
    # results = remove_anchor(get_results(mode, query_clip, query_gcn), None)[:top_k]
    results = remove_anchor(get_results(mode, query_clip, query_gcn), query_idx)[:top_k]
    # st.write("### 🎯 Recommended Items:")
    # for i, (idx, score) in enumerate(results):
    #     st.image(f"{image_dir}/{item_names[idx]}", caption=f"{item_names[idx]} (score: {score:.2f})", use_container_width=True)
